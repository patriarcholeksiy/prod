...
public class AuthController {
    ...
    public AuthController(UserMapper userMapper, UserService userService, JwtService jwtService, CountryService countryService, PasswordEncoder passwordEncoder, AuthenticationManager authenticationManager) {
    ...
    public ResponseEntity<?> registerUser(@RequestBody RegisterDTO registerDTO) {
        if (countryService.existsByAlpha2(registerDTO.getAlpha2())) {
            if (userService.checkValidRegistration(registerDTO)) {
                ...
                if (registerDTO.getIsPublic() == null)
                    user.setPublic(true);
                user.setPublic(registerDTO.getIsPublic());
